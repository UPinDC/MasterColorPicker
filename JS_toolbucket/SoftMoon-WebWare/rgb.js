// rgb.js Beta-1.0 release 1.0  12-31-2012  by SoftMoon Webware.
...
if (typeof Math.fmod !== 'function')
	Math.fmod=function(x, d)  {return x-Math.floor(x/d)*d;}        //     while (x>d) {x-=d;}  return x;}

if (typeof SoftMoon !== 'object')  SoftMoon=new Object;
if (typeof SoftMoon.WebWare !== 'object')   SoftMoon.WebWare=new Object;


/* The Regular Expressions (RegExp) defined below are here for your convienience, as well as being used within
	 ...
	 more convienient for your project’s code which uses these RexExps to have these as properties of the pre-existing
...
SoftMoon.defaultPalette='HTML';

if (typeof SoftMoon.palettes !== 'object')  SoftMoon.palettes=new Object;

SoftMoon.WebWare.Palette=function(palette)  {for (var c in palette)  {this[c]=palette[c];}}
SoftMoon.WebWare.Palette.prototype.getColor=function(clr)  { with (SoftMoon)  { var c, matches;
	clr=clr.replace( /^\s+/ , "").replace( /\s+$/ , "").toLowerCase();
	for (c in this)  { if (c==='getColor')  continue;
		if (!this[c].palette)  {
		...
		&&  c.toLowerCase()===matches[1].toLowerCase())
			return arguments.callee.call(this[c].palette, matches[2]);
		if (arguments.callee.requireSubindex!=='true'  &&  (matches=arguments.callee.call(this[c].palette, clr)))
			return matches;  }
	return null;  }  }


// This function will return an initially empty array.
// Once the index is asynchonously loaded via HTTP, the array will fill with HTTP-connect objects, one for each palette being loaded.
// Each HTTP-connect object has a property  .trying  which will become false once that palette is loaded (or if loading fails)
SoftMoon.WebWare.loadPalettes=function(path,   // ←required  ↓all optional on next line
																			 _addPalette, loadError, onMultiple, maxAttempts, timeoutDelay)  {
	if (typeof path != 'string'  ||  path==="")  path='color_palettes/';
	if (typeof _addPalette != 'function')  _addPalette=SoftMoon.WebWare.addPalette;
with (SoftMoon.WebWare)  {  //HTTP may be global or a property of SoftMoon.WebWare
	var files=new Array,
			connector=new HTTP(maxAttempts, timeoutDelay),
			paletteIndexConnection=HTTP.newConnection(path, 'Can not access color palettes: no HTTP service available.');
}
	if (paletteIndexConnection === null)  return false;
	paletteIndexConnection.onFileLoad=function()  {
		if (typeof this.responseText != 'string'  ||  this.responseText=="")  {files=null;  return;}
		var paletteIndex=this.responseText.split("\n");
		for (var i=0; i<paletteIndex.length; i++)  {
			if (paletteIndex[i]!=="")  {
				files[i]=HTTP.Connection(paletteIndex[i]);  // there should now be no reason we cannot create a new connection, so we don't check
				files[i].onFileLoad=_addPalette;
				files[i].loadError=loadError;
				files[i].onMultiple=onMultiple;  //if the server offers multiple choices for a file, this may require human interaction or otherwise.  You may pass in  HTTP.handleMultiple  for basic human intervention.
...
SoftMoon.WebWare.addPalette=function(_json_palette_)  {
	var json_palette =  (this === SoftMoon.WebWare) ? _json_palette_ : this.responseText;
	// JSON.parse resists single-quote-apostrophy in property names, and will not allow for custom methods, but may be a nessesary precaution for your implementation.
	if (typeof json_palette == 'string')  json_palette=eval("("+json_palette+")");   // json_palette=JSON.parse(json_palette);
	if (typeof json_palette == 'object')
		for (paletteName in json_palette)  {
			...
			if (typeof json_palette[paletteName].getColor == 'function')
				SoftMoon.palettes[paletteName].getColor=json_palette[paletteName].getColor;
			SoftMoon.palettes[paletteName].getColor.requireSubIndex=json_palette[paletteName].requireSubIndex;  }
	return json_palette;  }
...
SoftMoon.WebWare.ColorWheelColor=function(H,S,V, model, huesByDegrees)  {
	if (this===SoftMoon.WebWare)  throw new Error('ColorWheelColor is a constructor, not a function.');
	if (typeof huesByDegrees !== 'boolean')  huesByDegrees=!!SoftMoon.WebWare.rgb.huesByDegrees;
	this.model=model;
	this.hsv=new Array(H,S,V);
	this.hsv.huesByDegrees=huesByDegrees;
	this.h=H, this.s=S, this.v=V;
	this.hue=H, this.saturation=S, this.value=V;
	this.huesByDegrees=huesByDegrees;  }
SoftMoon.WebWare.ColorWheelColor.prototype.toString=function(format)  { var s;  if (typeof format != 'string')  format="";
	...
	||  (SoftMoon.WebWare.rgb.huesByDegrees  &&  format===""))
		s=Math.roundTo(parseFloat(this.h) * (this.huesByDegrees ? 1 : 360), 3)+'°';
	else
	if (format.match( /percent/i )
	||  (!SoftMoon.WebWare.rgb.huesByDegrees  &&  SoftMoon.WebWare.rgb.valuesAsPercent  &&  format===""))
		s=Math.roundTo(parseFloat(this.h) / (this.huesByDegrees ? 3.6 : .01), 1)+'%';
	else
	if (format.match( /factor/i )
	||  (!SoftMoon.WebWare.rgb.huesByDegrees  &&  !SoftMoon.WebWare.rgb.valuesAsPercent  &&  format===""))
		s=Math.roundTo(parseFloat(this.h) / (this.huesByDegrees ? 360 : 1), 3);
	else
		s=Math.roundTo(parseFloat(this.h), 3)+(this.huesByDegrees ? '°' : '');

	if (format.match( /percent/i )
	||  (SoftMoon.WebWare.rgb.valuesAsPercent  &&  format===""))
...
// The  rgb:(array)  is indexed  0,1,2  containing three (int) values correspondomg to the selected color’s red, green, and blue
...
SoftMoon.WebWare.rgb.getByte=getByteValue;  // this is here for user-added color-modelers to access
	// make sure the color’s value is an integer; and in the boundries of 0-255, if not, “reflect” it back or “truncate”.
...
//  for RGB (depending on SoftMoon.WebWare.rgb.valuesAsByte == false), CMY, CMYK, HSL, HSV and HCG color models,
//  depending on the flag below,
...
function getHuePercent(h)  {
	...
	||  (SoftMoon.WebWare.rgb.huesByDegrees  &&  (typeof h != 'string'  ||  h.substr(-1)!=='%')))  ?
		((Math.fmod(parseFloat(h), 360)/3.6)+'%')  :  h;  }
...
SoftMoon.WebWare.rgb.to.hex=function(rgb)  {return _2hex(rgb[0])+_2hex(rgb[1])+_2hex(rgb[2]);}
function _2hex(d)  {return ((Math.round(d)<16) ? "0" : "") + Math.round(d).toString(16).toUpperCase();}
...
SoftMoon.WebWare.rgb.to.hsv=SoftMoon.WebWare.rgb.to.hsv; // = function(rgb) {return this.hsb(rgb, 'HSV');}
 ...
 *  & Chroma calculated in the same fasion as Saturation,
 *  with Hue as the angle around the cyclinder (red at 0°, green at 120°, blue at 240°),
 ...
 *   As viewed, you can not tell the “height” dimention of the slice, so the progression of Chroma appearance
 ...
 *  to a long sharp-pointed “spear” quckly curving to a disk at the other end end when Curve becomes very small (¡always! > 0).
 ...
 *  from the pure Hue to the Gray shade (only in HCGC the Chroma line is layed over the curved surface).
 ...
 *  This means that the edges of an HCG or HCGC cylinder remain consistant thoughout the height of the cylinder,
 *  as opposed to the HSV & HSL cylinders inwhich the “shade” of the Hue varies with the height.
